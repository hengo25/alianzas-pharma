...
ruta_llave = os.path.join(app.root_path, "llave-firebase.json")
...
def _decodificar_nombre_especial(nombre):
    """Convierte nombres guardados con secuencias tipo #U00C1 a Unicode."""
    import re
    def repl(match):
        try:
    ...
    return re.sub(r"#U([0-9A-Fa-f]{4})", repl, nombre)


def _buscar_archivo_static(nombre):
    """Busca un archivo de imagen de forma tolerante a mayúsculas, espacios y encoding."""
    if not nombre:
        return None

    nombre = unquote(str(nombre)).strip().replace('\\', '/')
    nombre = posixpath.basename(urlparse(nombre).path)
    if not nombre:
        return None

    candidatos = [nombre, _decodificar_nombre_especial(nombre)]

    for candidato in candidatos:
        ruta = os.path.join(STATIC_DIR, candidato)
        if os.path.isfile(ruta):
            return candidato

    def normalizar(valor):
        valor = _decodificar_nombre_especial(unquote(str(valor)))
    ...
    for archivo in archivos:
        ...
        if os.path.isfile(ruta) and normalizar(archivo) == buscado:
            return archivo

    # Segunda oportunidad: compara sin diferencias de espacios.
    buscado_sin_espacios = buscado.replace(' ', '')
    for archivo in archivos:
        ruta = os.path.join(STATIC_DIR, archivo)
        if os.path.isfile(ruta) and normalizar(archivo).replace(' ', '') == buscado_sin_espacios:
            return archivo

    return None


def preparar_imagen_producto(valor):
    """Devuelve una URL segura para imágenes locales o conserva URLs externas."""
    if not valor:
        return url_for('servir_imagen_producto', nombre='logo.jpeg')

    valor = str(valor).strip()
    if valor.startswith(('http://', 'https://', 'data:')):
        return valor

    nombre = _buscar_archivo_static(valor)
    if nombre:
        return url_for('servir_imagen_producto', nombre=nombre)

    return url_for('servir_imagen_producto', nombre='logo.jpeg')


@app.route('/producto-imagen/<path:nombre>')
def servir_imagen_producto(nombre):
    """Sirve las fotos desde static incluso cuando Vercel no resuelve /static directamente."""
    archivo = _buscar_archivo_static(nombre)
    if not archivo:
        archivo = 'logo.jpeg'
...
def inicio():
    ...
    for doc in db.collection("productos").stream():
        p = doc.to_dict()
        lista.append({"id": doc.id, "nombre": p.get("nombre", "Sin nombre"), "precio": int(p.get("precio", 0)), "imagen": preparar_imagen_producto(p.get("imagen")), "existencias": int(p.get("existencias", 0))})
...
def administrador():
    ...
    if request.method == 'POST':
        if nombre and precio:
            ...
            if foto and foto.filename != '':
                foto.save(os.path.join(STATIC_DIR, foto.filename))
                ruta_imagen = preparar_imagen_producto(foto.filename)
            else:
    ...
    for doc in db.collection("productos").stream():
        ...
        lista_completa.append({
            ...
            "imagen": preparar_imagen_producto(p.get("imagen")), 
...
def actualizar_stock(id):
    ...
    if 'nombre' in request.form:
        ...
        if foto and foto.filename != '':
            foto.save(os.path.join(STATIC_DIR, foto.filename))
            update_data["imagen"] = preparar_imagen_producto(foto.filename)
